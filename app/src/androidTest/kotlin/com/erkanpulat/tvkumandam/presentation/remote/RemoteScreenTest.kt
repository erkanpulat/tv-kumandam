...
class RemoteScreenTest {
    ...
    fun arcelikDeckPreservesTypedOrderAndKeepsPowerOutsideDeck() {
        ...
        composeRule.onNodeWithTag("quick_action_0")
            .assertIsDisplayed()
            .assertTextContains("Kaynak")
            .performClick()
        assertEquals(RemoteAction.Command(RemoteCommand.SOURCE), sentAction)
    ...
    @Test
    fun macroProgressLocksEveryTransmissionAndCancelRemainsEnabled() {
        var cancelled = false
        var haptics = 0
        setRemoteContent(
            state = stateFor(
                ...
                transmissionState = TransmissionState.Macro(
                    ...
                    macroId = TEST_HDMI_MACRO.id,
                    macroName = TEST_HDMI_MACRO.name,
                    completedSteps = 3,
                    totalSteps = 9,
                ),
            ),
    ...
    @Test
    fun userMacroDispatchesMacroActionAndExplainsDuration() {
        ...
        composeRule.onNodeWithTag("quick_action_0")
            ...
            .performClick()
        assertEquals(RemoteAction.Macro(TEST_HDMI_MACRO.id), macroAction)
    }
    ...
    private fun allCapabilitiesProfile(template: RemoteLayoutTemplate): RemoteProfile {
        val evidence = ProfileEvidence(EvidenceTier.SOURCE_VERIFIED, "Remote screen fixture")
        ...
        return RemoteProfile(
            ...
            inputCapability = if (template == RemoteLayoutTemplate.DIRECT_INPUT) {
                ...
                InputCapability.sourceOnly()
            },
        )
    }

    private fun smartMediaProfile(extraCommands: Set<RemoteCommand>): RemoteProfile {
        val evidence = ProfileEvidence(EvidenceTier.SOURCE_VERIFIED, "Remote section fixture")
