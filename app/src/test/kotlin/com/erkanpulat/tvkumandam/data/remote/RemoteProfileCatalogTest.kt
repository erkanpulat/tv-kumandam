...
class RemoteProfileCatalogTest {
    ...
    fun `bundled profiles identify exact families evidence layouts and input strategies`() {
        ...
        assertEquals(InputStrategy.SOURCE_MENU_MACROS, arcelik.inputCapability.strategy)
        assertEquals(
        ...
        assertEquals(InputStrategy.SOURCE_MENU_MACROS, beko.inputCapability.strategy)
        assertEquals(
            listOf(RemoteAction.Command(RemoteCommand.SOURCE)),
        ...
        assertEquals(InputStrategy.SOURCE_MENU_MACROS, grundig.inputCapability.strategy)
    }
    ...
    @Test
    fun `RC-YC1 profiles expose the reliable HDMI1 navigation sequence`() {
        val expectedCommands = listOf(RemoteCommand.SOURCE) +
            List(7) { RemoteCommand.DOWN } +
            RemoteCommand.OK
        val expectedDelays = listOf(1_000L) +
            List(6) { 300L } +
            listOf(500L, 0L)
        ...
        listOf(
            ...
            assertEquals(expectedDelays, sequence.steps.map { it.delayAfterMillis })
            assertTrue(sequence.steps.all { it.transmissionCount == 2 })
            assertTrue(sequence.steps.all { it.transmissionGapMillis == 89L })
            assertEquals(setOf(RemoteShortcut.HDMI1), profile.supportedShortcuts)
        }
    }
