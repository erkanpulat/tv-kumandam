...
class RemoteProfileTest {
    ...
    @Test
    fun `discrete HDMI input metadata exposes only documented commands`() {
    ...
    @Test
    fun `source-only input metadata carries no discrete command`() {
        val profile = validProfile()
    ...
    @Test
    fun `profile rejects discrete input metadata without a matching binding`() {
        assertThrows(IllegalArgumentException::class.java) {
            validProfile(
                inputCapability = InputCapability.discreteCommands(setOf(RemoteCommand.HDMI1)),
            )
